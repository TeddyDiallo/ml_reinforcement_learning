...
def main():
    ...
    for run in range(numRuns):
        ...
        for episode in range(numEpisodes):
            ...
            for s in range(maxSteps):
                ...
                qtable[state[0][0], action] = qtable[state[0][0], action] + learningRate * (
                        reward + discountRate * np.max(qtable[newState, :]) - qtable[state[0][0], action])
    ...
    plt.title('Averaged Q-Learning Performance: Total Rewards per Episode')
    plt.legend()
    plt.show()
