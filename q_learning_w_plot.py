...
#this program will use q-learning 
def main():
    ...
    episode_rewards = []  # Initialize a list to store the total rewards per episode for plotting
    for episode in range(numEpisodes):
        ...
        for s in range(maxSteps):
            ...
            qtable[state[0][0],action] = qtable[state[0][0],action] + learningRate * (reward + discountRate * np.max(qtable[newState,:])-qtable[state[0][0],action])
    ...
    print("Q-LEARNING training completed using", numEpisodes, "episodes")
    input("Press Enter to view trained agent")
    
    # After completing all episodes, plot the cumulative rewards per episode
    plt.figure(figsize=(10, 6))
    plt.plot(episode_rewards, label='Total Rewards per Episode')
    plt.xlabel('Episode')
    plt.ylabel('Total Reward')
    plt.title('Q-Learning Performance: Total Rewards per Episode')
    plt.legend()
    plt.show()
